...
interface UseAppointmentReturn {
  ...
  getAppointments: (props: GetAllAppointmentsProps) => Promise<Appointment[]>;
...
export const useAppointment = (): UseAppointmentReturn => {
  ...
  const getActiveAppointments = async (
    ...
    const userRole = userSnapshot.data().role;
    const invalidStates: AppointmentState[] = ["FINISHED"];
    if (userRole === "ADMIN" || userRole === "MOD") {
      ...
      return appointments.filter((app) => !invalidStates.includes(app.state));
    } else {
  ...
  const convertAppsForHomePage = (appointments: Appointment[]) => {
    ...
    for (let i = 0; i < appointments.length; i++) {
      const currApp = appointments[i];
      const services: ACPService[] = [];
      for (let j = 0; j < currApp.services.length; j++) {
  ...
  const formatAppointments = (appointments: Appointment[]) => {
    const ans: FullCalendarAppointment[] = [];
    for (let i = 0; i < appointments.length; i++) {
      const currApp = appointments[i];
