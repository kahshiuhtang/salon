...
export default function RequestsPage() {
  ...
  const [filteredRequests, setFilteredRequests] = useState<AppointmentWithClientName[]>([])
  ...
  async function fetchRoleAndRequests() {
    try {
      ...
      const appsWithNames = await Promise.all(
        apps.map(async (app) => {
          const { firstName, lastName } = await getNameFromId({ userId: app.ownerId })
  ...
  useEffect(() => {
    const filtered = requests.filter((request) => {
      const nameMatch = request.clientName.toLowerCase().includes(searchTerm.toLowerCase())
      const statusMatch = statusFilter === "ALL" || request.state === statusFilter
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {currRole === "MOD" && (
          <div className="text-center">
            ...
            <p className="text-gray-600">Welcome, Moderator. Your content will be displayed here.</p>
          </div>
        )}
        {currRole === "USER" && (
          <div className="text-center">
            <h1 className="text-3xl font-bold mb-4">User Dashboard</h1>
            <p className="text-gray-600">Welcome, User. Your appointments and requests will be displayed here.</p>
          </div>
        )}
      </main>
    </div>
